...
internal static class SeriesEpisodeMuxArgumentBuilder
{
        ...
        ISet<string> audioAlreadyEmittedForFile)
    {
        ...
        [
            ...
            "--original-flag",
            $"{primaryVideoTrackId}:{ResolveOriginalFlag(primaryVideo.LanguageCode, plan.OriginalLanguage, plan.OutputFilePath)}"
        ]);

        AppendAudioTrackMetadata(arguments, audioSources, plan.OriginalLanguage, plan.OutputFilePath);
        ...
        ISet<string> audioAlreadyEmittedForFile)
    {
        foreach (var videoSource in plan.VideoSources.Skip(1))
        {
            ...
            [
                ...
                $"{trackId}:mono",
                "--original-flag",
                $"{trackId}:{ResolveOriginalFlag(videoSource.LanguageCode, plan.OriginalLanguage, plan.OutputFilePath)}"
            ]);
            AppendAudioTrackMetadata(arguments, audioSources, plan.OriginalLanguage, plan.OutputFilePath);
    ...
    private static void AppendAudioDescription(List<string> arguments, SeriesEpisodeMuxPlan plan)
    {
        ...
        foreach (var audioDescriptionSource in plan.AudioDescriptionSources)
        {
            ...
            [
                ...
                $"{adTrackId}:yes",
                "--original-flag",
                $"{adTrackId}:{ResolveOriginalFlag(audioDescriptionSource.LanguageCode, plan.OriginalLanguage, plan.OutputFilePath)}",
        ...
        string? seriesContext)
    {
        foreach (var audioSource in audioSources)
        {
            ...
            [
                ...
                "--original-flag",
                $"{trackIdText}:{ResolveOriginalFlag(audioSource.LanguageCode, seriesOriginalLanguage, seriesContext)}"
            ]);
        }
    }
    ...
    private static void AppendSubtitles(List<string> arguments, SeriesEpisodeMuxPlan plan)
    {
        foreach (var subtitle in plan.SubtitleFiles)
        {
            ...
            AppendExternalSubtitle(arguments, subtitle, plan.OriginalLanguage, plan.OutputFilePath);
        }
    }
        ...
        int embeddedTrackId)
    {
        ...
        [
            ...
            "--original-flag",
            $"{embeddedTrackIdText}:{ResolveOriginalFlag(subtitle.LanguageCode, plan.OriginalLanguage, plan.OutputFilePath)}",
            plan.ResolveRuntimeFilePath(subtitle.FilePath)
        ]);
    }

    private static void AppendExternalSubtitle(
        List<string> arguments,
        SubtitleFile subtitle,
        string? seriesOriginalLanguage,
        string? seriesContext)
    {
        ...
        [
            ...
            "--original-flag",
            $"0:{ResolveOriginalFlag(subtitle.LanguageCode, seriesOriginalLanguage, seriesContext)}",
            subtitle.FilePath
        ]);
    }
    ...
    {
        return SeriesOriginalLanguageRules.ResolveOriginalFlag(trackLanguageCode, seriesOriginalLanguage);
    }

    /// <summary>
    /// Bestimmt den Wert des <c>--original-flag</c> inklusive serienbezogener Ausnahmen.
    /// </summary>
    internal static string ResolveOriginalFlag(
        string? trackLanguageCode,
        string? seriesOriginalLanguage,
        string? seriesContext)
    {
        return SeriesOriginalLanguageRules.ResolveOriginalFlag(trackLanguageCode, seriesOriginalLanguage, seriesContext);
    }
